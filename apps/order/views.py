...
class OrderPlaceView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        user = request.user
        # 获取参数sku_ids
        sku_ids = request.POST.getlist('sku_ids')  # [1, 25]
        ...
        for sku_id in sku_ids:
            ...
            # 动态给sku增加count,保存购买商品数量
            sku.count = int(count)
            # 动态给sku增加count,保存购买商品的小计
            sku.amount = amount
            # 追加
            skus.append(sku)
            # 累加计算商品的总件数和总价格
...
# 前端传递参数:地址id(addr_id) 支付方式(pay_method)  用户要购买的商品id字符串(sku_ids)
...
class OrderPayView(View):
    ...
    def post(self, request):
        ...
        alipay = AliPay(
            appid="2016102600767081",  # 应用id(这里使用的是沙箱id)
        ...
        total_pay = order.total_price + order.transit_price  # Decimal这里类型需要转换成str
